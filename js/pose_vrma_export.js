...
export function buildKeyframePanel(editor, getVrmBuffer) {
    ...
    const titleEl = el("span", { style: "font-size:11px;font-weight:bold;color:#9aa;margin-right:4px;" }, "🎬 Keyframes");
    ...
    toolbar.append(titleEl, addFromLibBtn, addCurrentBtn, hint);

    // ---- コンテンツ(キーフレーム一覧 / ライブラリ選択サブビュー) ----
    const content = el("div", {
        style: "max-height:110px;overflow-y:auto;padding:6px 12px;box-sizing:border-box;flex-shrink:0;",
    });
    ...
    const previewPanel = el("div", {
        style: "display:flex;align-items:center;gap:8px;padding:6px 12px;" +
               "background:#13131e;border-top:1px solid #2a2a4a;flex-shrink:0;",
    });
    ...
    panel.append(toolbar, content, previewPanel, footer);
    ...
    function renderList() {
        ...
        if (keyframes.length === 0) {
            listEl.appendChild(el("div", { style: "font-size:11px;color:#666;padding:6px 0;" },
